...
find_clusters <- function(obj,
                          ...
                          resolution = 0.8,
                          verbose = FALSE) {
  # Peaks methods need slight variation
...
multiple_cluster <- function(obj,
  ...
  # (returns a data frame with a single column named "cells")
  clusters <- data.frame(cell=colnames(obj))

  # Get samples
  samples <- n_samples(n, clusters$cell, size = size)
  ...
  for (idx in samples) {
    ...
    # Clean the data frame storing new clustering info
    new.clusters <- data.frame(small_obj$seurat_clusters) %>%
        rownames_to_column(var="cell")

    # Rename the columns
    colnames(new.clusters) <- c("cell", paste0("value_", j))

    # Join two data tables
    clusters <- merge.data.frame(clusters,
                                 new.clusters,
                                 by="cell",
                                 all.data=TRUE)
...
# group_scroes modified by Mira
#' @param dt Data table 
#' @param clusters Integer or numeric vector
group_scores <- function(dt, clusters) {

    colnames(dt) <- as.character(clusters)
    data <- dt[, "cell_1" := as.character(clusters)]
    data <- data.table::melt(data,
                             id.vars="cell_1",
                             variable.name="cell_2",
                             value.name="percent")
    data <- data %>%
...
# group_sil modified by Mira
#' @param sil silhouette obj
#' @param res resolution
group_sil <- function(sil, res) {

    sil <- as.data.table(sil[,]) %>%
        group_by(cluster) %>%
