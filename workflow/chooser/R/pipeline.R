...
multiple_cluster <- function(
  ...
  # Initialize data table (returns a data frame with a single column named "cells")
  clusters <- data.table(cell=Seurat::Cells(obj))
  ...
  for (idx in samples) {
    ...
    new.clusters <- data.frame(Seurat::Idents(small_obj)) %>%
    ...
    # Update from data table to data frame
